...
const CONTRACT_ADDRESS4 = "0x5eBC6Aa5BE44e8e1E8671bD09Ae338aD676Ac5eB"
...
const CONTRACT_ABI4 = require('../constant/chronic.json');
...
const contract4 = new ethers.Contract(CONTRACT_ADDRESS4, CONTRACT_ABI4, wallet);

routerB.get('/blockchain', (req, res) => {
  res.status(200).send("blockchain route is working!");
});

// ─── Store CKD + Prediction ───────────────────────────────
routerB.post(
  '/blockchain/store-ckd',
  authenticateUser,
  async (req, res) => {
    try {
      console.log('Received request to store CKD on blockchain…', req.body);
      ...
      if (!Array.isArray(dataArray)) {
        return res
          .status(400)
          .json({ error: 'Expected an array of CKD data objects' });
      }
      ...
      // now 25 fields, prediction is last
      const requiredFields = [
        'age', 'bloodPressure', 'specificGravity', 'albumin', 'sugar',
        'redBloodCells', 'pusCell', 'pusCellClumps', 'bacteria', 'bloodGlucoseRandom',
        'bloodUrea', 'serumCreatinine', 'sodium', 'potassium', 'haemoglobin',
        'packedCellVolume', 'whiteBloodCellCount', 'redBloodCellCount',
        'hypertension', 'diabetesMellitus', 'coronaryArteryDisease',
        'appetite', 'pedaEdema', 'aanemia','prediction'
      ];

      let ckdTransactionHashes = [];
      ...
      for (const data of dataArray) {
        ...
        // build the ordered values array
        const vals = requiredFields.map(f => data[f]);

        // call updated storeCKDData(vals) that now expects 25 elements
        const tx = await contract4.storeCKDData(...vals);
        await tx.wait();
        ckdTransactionHashes.push(tx.hash);
      }

      await User.updateOne(
        { username },
        { $push: { ckdTransactionHashes: { $each: ckdTransactionHashes } } }
      );

      const count = await contract4.getRecordCount();    // BigInt
      if (count > 0n) {
        const index = count - 1n;   

        // idx is now a normal number
        const raw = await contract4.getCKDData(index);

        const storedData = {};
        requiredFields.forEach((f) => {
          storedData[f] = raw[f].toString();
        });
        console.log("Stored Data:", storedData);
        res.status(200).json({
          message: 'CKD data stored, but no records found.',
          storedData
        });
      }else {
      ...
      console.error('Error storing CKD data:', err);
      return res
        .status(500)
        .json({ error: 'Failed to store CKD data on blockchain' });
    }
  }
);
...
// ─── Fetch All CKD + Prediction ────────────────────────────
routerB.get(
  '/blockchain/ckd/all-user-records',
  authenticateUser,
  async (req, res) => {
    try {
      const username = req.user.username;
      ...
      const txHashes = user.ckdTransactionHashes || [];
      if (!txHashes.length) {
      ...
      const records = await Promise.all(
        txHashes.map(async txHash => {
          ...
          for (const log of receipt.logs) {
            try {
              parsed = contract4.interface.parseLog(log);
              if (parsed.name === 'DataStored') break;
            } catch { }
          }
          if (!parsed) return null;

          const idx = parsed.args[0];
          const onChain = await contract4.getCKDData(idx);

          // convert all fields to string
          const obj = {};
          fields.forEach(f => {
            obj[f] = onChain[f].toString();
          });
          return obj;
        })
      );
      ...
      console.error('Error fetching CKD records:', err);
      res.status(500).json({ error: 'Failed to fetch CKD records' });
    }
  }
);
